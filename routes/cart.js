...
router.post('/add-to-cart', protect, async (req, res, next) => {
    ...
    try {
        ...
        if (!cart) {
            cart = await CartItem.create({ userId: userId, books: [{ bookId: bookId, quantity: 1 }] });
        } else {
            let existingProduct = cart.books.find(item => String(item.bookId) === String(bookId));
...
router.get('/',protect, async (req, res) => {
    const userId = req.user._id; 
    try {
        const cart = await CartItem.findOne({ userId: userId }).populate('books.bookId');
        if (!cart) {
...
router.delete('/delete-item/:id', protect,async (req, res) => {
    ...
    try {
        ...
        cart.books = cart.books.filter(item => item.bookId.toString() !== itemId.toString());
        await cart.save();
